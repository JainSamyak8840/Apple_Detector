...
def select_image():
    ...
    if len(path)>0:
        ...
        edged=cv2.cvtColor(process(org_image),cv2.COLOR_BGR2RGB)
        proc=cv2.cvtColor(edged,cv2.COLOR_BGR2RGB)
        cv2.imwrite(CWD_PATH +'/output/'+img_name,proc)
